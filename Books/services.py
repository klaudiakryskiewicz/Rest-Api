    ...
    def handle(self, *args, **options):
        with urllib.request.urlopen("https://www.googleapis.com/books/v1/volumes?q=Hobbit") as url:
            ...
            for book_item in data:
                ...
                k['published_date'] = book_item.get('published_date')
                k['average_rating'] = book_item.get('average_rating')
                k['ratings_count'] = book_item.get('ratings_count')
                k['thumbnail'] = book_item.get('thumbnail')
                book, created = Book.objects.get_or_create(**k)
                author_list = book_item.get('author')
                category_list = book_item.get('category')
